...
matlab_cmd = @( kk ) ['\"cd ' pa.codePath 'code/;' 'jadd_path; load(''' pa.pfj 'job_' num2str( kk, '%.4d') ''');process_job(''' pa.pfj 'job_' num2str(kk,'%.4d') ''', ''' pa.pfj 'ans_' num2str(kk,'%.4d') ''', ''' pa.pfj 'f.mat'', ' num2str(pa.max_iter) ');exit;\"'];
