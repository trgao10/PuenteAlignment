function write_align_pca_files(filepath, indNames, eigenvector, score, eigenvalue, explained)
...
pca_folder_path = fullfile(filepath, 'align_coord_pca');
touch(pca_folder_path);
...
title('Align Coordinate PCA');
xlabel('PC1');
ylabel('PC2');
saveas(fig, fullfile(pca_folder_path, 'pca_score_plot.png'));
saveas(fig, fullfile(pca_folder_path, 'pca_score_plot.eps'));
...
writetable(sTable,fullfile(pca_folder_path, 'pca_scores.csv'));

eTable = array2table([(1:vars)', eigenvector], ...
	'VariableNames', ['Variable', pcNames]);
writetable(eTable, fullfile(pca_folder_path, 'pca_eigenvectors.csv'));

vTable = table((1:nPC)', eigenvalue, explained, ...
	'VariableNames',{'PC', 'Eigenvalue', 'Percent_variance_explained'});
writetable(vTable, fullfile(pca_folder_path, 'pca_variance.csv'));
