...
class Task:
  ...
  def getacol(self,j_,subj_,valj_):
    ...
    nzj_ = ctypes.c_int32()
    nzj_ = ctypes.c_int32()
    if subj_ is not None and len(subj_) < self.getj_():
    ...
    if subj_ != None:
      ...
      _subj_copyarray = False
      _subj_tmp = None
    if valj_ is not None and len(valj_) < self.getj_():
  ...
  def getarow(self,i_,subi_,vali_):
    ...
    nzi_ = ctypes.c_int32()
    nzi_ = ctypes.c_int32()
    if subi_ is not None and len(subi_) < self.geti_():
    ...
    if subi_ != None:
      ...
      _subi_copyarray = False
      _subi_tmp = None
    if vali_ is not None and len(vali_) < self.geti_():
