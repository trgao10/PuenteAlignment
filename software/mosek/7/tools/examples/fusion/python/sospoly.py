...
def diff(M, x):
    ...
    tmp = Variable.reshape(x.slice(1,n+1),NDSet(1,n))
    M.constraint(Expr.sub(u, Expr.mulElm(DenseMatrix([[float(i) for i in range(1,n+1)]]), tmp)), 
                     Domain.equalsTo(0.0))
    return u
...
def fitpoly(data, n):
    ...
    M.constraint(Expr.mul(DenseMatrix(A), x), Domain.equalsTo(b))
